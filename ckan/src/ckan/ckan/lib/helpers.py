...
from  ckan.contadores.modelo.gestionDB import getPaquetesjson,getDataset,getPaquetesjsonBI
import ckan.contadores
from ckan.contadores.main.contador import getContador_viewDescargas
import tzlocal
import pprint
import copy
import uuid
...
from ckan.types import Context, DataDict, Response
...
#def contruir_Api(context: Context, data_dict: DataDict) -> ActionResult.PackageShow:
def contruir_Api() -> jsonData_list:    
    dataList=[]
    data={}
    data["name"]={}
    result=getPaquetesjson()
   
    
    
    #data["name"]='{}'.format(getPaquetesjson())
    dataList.append(data)
   
    headers = {"Content-Type": "application/json"}  
    return dataList

def contruir_PowerBI_Api() -> jsonData_list:    
    dataList=[]
    data={}
    data["name"]={}
    result=getPaquetesjsonBI()
...
def obtener_contador_package(package_id):
    ...
    registros = session.query(Contadores).filter_by(
...
def obtener_contador_resource(package_id, resource_id):
    ...
    registro = session.query(Contadores).filter_by(
        package_Id=package_id,
    ...
    return {
        "views": registro.contVistas,
        "downloads": registro.contDownload
    }
...
def guardar_contador(package_id, resource_id, tipo):
    ...
    registro = session.query(Contadores).filter(
        and_(
            Contadores.package_Id == package_id,
            Contadores.source_Id == resource_id
        )
    ).first()
    ...
    log.warning("[helplers][guardar_contador] registro despues de contar %s",registro)    
    
    session.commit()  

@core_helper
def get_contadores(resource_id):

    """
    Funcion que retorna la cantidad de descargas y vistas que ha tenido el recurso 

    args

    dict[] con la informacion del recurso pedido

    Retorna una lista con la informacion del registro de contador para ser visualizado desde la pagina html
    """
    resultado={}      
    contadorList=[]
    data={"sourceId":resource_id}
    resultado=getContador_viewDescargas(data)
   
    contadorList.append(resultado)
...
@core_helper
def get_group(org: Optional[str] = None,
                     include_datasets: bool = False) -> dict[str, Any]:
    if org is None:
        return {}
    try:
