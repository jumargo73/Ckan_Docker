...
def read(package_type: str, id: str, resource_id: str) -> Union[Response, str]:
    ...
    actualizarContador(package,"Visualizacion")
...
def download(package_type: str,
    ...
    if rsc.get(u'url_type') == u'upload':
        ...
        print(data)
        actualizarContador(data,"Download")
...
class CreateView(MethodView):
    def post(self, package_type: str, id: str) -> Union[str, Response]:
        ...
        if save_action == u'go-metadata':
            ...
            actualizarContador(data_dict,"Create")
            return h.redirect_to(u'{}.read'.format(package_type), id=id)
    ...
    def get(self,
        ...
        log.critical( f"CKAN_INI: { config.get('CKAN_INI') }")
        log.critical( f"CKAN_CONFIG: { config.get('CKAN_CONFIG') }")
        log.critical( f"_file: { config.get('file_') }")
        log.critical( f"max_resource_size: { config.get('ckan.max_resource_size') }")
...
def actualizarContador(package,context):

    try:
        """
        Funcion que actualiza el contador dependiente si es por visualizacion o descarga

        args:
        dict[] informacion del paquete donde encuentras la informacion del nombre del grupo y recurso
    
        """
        isVacioTematica=len(package["id"])
        isVacioResouce=len(package["resources"][0]['id'])
        
        if (isVacioTematica>0):
            if (isVacioResouce>0):
                h.guardar_contador(package["id"],package["resources"][0]["id"],context)                   
            else:
               pass
        else:
            pass   
